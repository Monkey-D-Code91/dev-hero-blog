...
2. Cover generate (`generate-cover.mjs`) e collegate nel frontmatter (`cover` + `coverAlt`).
3. Se serve il giro di feedback: PDF con `generate-feedback-pdf.py`.
4. Togli `draft: true` su IT ed EN.
5. `npm run build` **in locale** per validare (nel sandbox può non girare: vedi nota ambiente).
6. Commit + PR su GitHub.
7. Condivisione su LinkedIn: carousel (`generate-carousel.mjs`) e/o post con **link nel primo
   commento**. Un gancio forte aiuta (es. per il pezzo sulla code review: "quindici minuti di lavoro,
   quattro ore di review").
8. Aggiorna `HANDOFF.md` se lo stato della pipeline è cambiato.
